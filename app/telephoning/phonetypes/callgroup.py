...
from app.models.crud.asterisk import create_or_update_asterisk_dialplan_callgroup, delete_asterisk_dialplan_entry
...
class CallGroupFields(BaseModel):
    ...
    participants: str = Field(pattern=fr"^\d{{{settings.EXTENSION_DIGITS}}}(\s*,\s*\d{{{settings.EXTENSION_DIGITS}}})*$")
...
class CallGroup(PhoneFlavor):
    ...
    def on_extension_create(self, session, asterisk_session, user, extension):
        create_or_update_asterisk_dialplan_callgroup(session, asterisk_session, user, extension)
